...
})({"38iX0":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "53a13086f5bf84e4";
"use strict";
...
},{}],"igkur":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
//Changes the appearence of the cart depending on whether the item is there or not
parcelHelpers.export(exports, "changeCartAppear", ()=>changeCartAppear);
//Draws a newly added product card to cart
parcelHelpers.export(exports, "draWNewCard", ()=>draWNewCard);
// Changes TOTAL sum in the cart
parcelHelpers.export(exports, "changeTotalSum", ()=>changeTotalSum);
...
function changeCartAppear(cart) {
    if (cart.length === 0) {
...
function draWNewCard(idItem, needSizes, size) {
    if (!localStorage.hasOwnProperty("cart")) return;
    cards = JSON.parse(localStorage.getItem("cart"));
    changeCartAppear(cards);
    for (const elem of cards)if (elem.id === idItem) {
        if (needSizes) {
            if (elem.size === size) (0, _addToCartJs.generateCartItem)(elem.title, elem.currentPrice, elem.imgSrc, elem.count, elem.id, needSizes, size);
        } else (0, _addToCartJs.generateCartItem)(elem.title, elem.currentPrice, elem.imgSrc, elem.count, elem.id, needSizes);
    }
}
...
function getCards() {
    ...
    cards = JSON.parse(localStorage.getItem("cart"));
    changeCartAppear(cards);
    for(let i = 0; i < cards.length; i++)if (cards[i].hasOwnProperty("size")) (0, _addToCartJs.generateCartItem)(cards[i].title, cards[i].currentPrice, cards[i].imgSrc, cards[i].count, cards[i].id, cards[i].needSizes, cards[i].size);
    else (0, _addToCartJs.generateCartItem)(cards[i].title, cards[i].currentPrice, cards[i].imgSrc, cards[i].count, cards[i].id, cards[i].needSizes);
    changeTotalSum();
    (0, _basketCounterJs.changeCartCounter)();
}
...
},{"./basketCounter.js":"54VTX","./addToCart.js":"9FVlh","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"54VTX":[function(require,module,exports) {
// Changes the number on the cart counter
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "changeCartCounter", ()=>changeCartCounter);
// Increases by one the number on the cart counter
parcelHelpers.export(exports, "increaseCartCount", ()=>increaseCartCount);
// Reduces by one the number on the cart counter
parcelHelpers.export(exports, "reduceCartCount", ()=>reduceCartCount);
...
},{}],"9FVlh":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
//Checks to see if the product card is already in the local storage
parcelHelpers.export(exports, "existInStorage", ()=>existInStorage);
//Adds product card to local storage
parcelHelpers.export(exports, "addCardtoStorage", ()=>addCardtoStorage);
//Draws product card in the cart
parcelHelpers.export(exports, "generateCartItem", ()=>generateCartItem);
//Increases by one quantity of item in the cart
parcelHelpers.export(exports, "increaseItemCount", ()=>increaseItemCount);
//Pop-up
parcelHelpers.export(exports, "addtoCartPopUp", ()=>addtoCartPopUp);
...
function existInStorage(id, needSizes, size) {
    let cart = JSON.parse(localStorage.getItem("cart"));
    if (needSizes) {
        for (const elem of cart){
            if (elem.id === id && elem.size === size) return true;
        }
        return false;
    } else {
        for (const elem of cart){
...
function addCardtoStorage(title, currentPrice, id, source, needSizes, size) {
    let cart = JSON.parse(localStorage.getItem("cart"));
    if (needSizes) {
...
function generateCartItem(title, currentPrice, source, count, id, needSizes, size) {
    const wrapper = document.querySelector(".shopping__bag-card--wrapper");
    const card = (0, _elemCreatorJs.createElementTag)("div", [
        "shopping__bag-card"
    ], {});
    wrapper.append(card);
    if (size !== undefined) card.id = `${id}-${size}`;
    else card.id = `${id}-1`;
    const infoWrap = (0, _elemCreatorJs.createElementTag)("div", [
        "card__info--wrapper"
    ], {});
    ...
    const cardImg = (0, _elemCreatorJs.createElementTag)("img", [], {
        src: source,
        alt: title
    });
    imgWrap.append(cardImg);
    const cardText = (0, _elemCreatorJs.createElementTag)("div", [
        "card__text"
    ], {});
    infoWrap.append(cardText);
    const cardTitle = (0, _elemCreatorJs.createElementTag)("p", [
        "card__title"
    ], {});
    cardTitle.innerText = title;
    cardText.append(cardTitle);
    if (needSizes) {
        const selectedSize = (0, _elemCreatorJs.createElementTag)("p", [
            "card__sizes"
        ], {});
        selectedSize.innerText = `Size: ${size}`;
        cardText.append(selectedSize);
    }
    const priceWrap = (0, _elemCreatorJs.createElementTag)("div", [
        "card__price--wrapper"
    ], {});
    card.append(priceWrap);
    const cardPrice = (0, _elemCreatorJs.createElementTag)("p", [
        "card__price"
    ], {});
    cardPrice.innerText = currentPrice * count + "$";
    priceWrap.append(cardPrice);
    const cardCount = (0, _elemCreatorJs.createElementTag)("div", [
        "card__count"
    ], {});
    priceWrap.append(cardCount);
    const minus = (0, _elemCreatorJs.createElementTag)("button", [
        "card__count-minus"
    ], {});
    minus.innerText = "-";
    cardCount.append(minus);
    minus.addEventListener("click", ()=>{
        if (size !== undefined) reduceItemCount(id, needSizes, size);
        else reduceItemCount(id, needSizes);
    });
    const countValue = (0, _elemCreatorJs.createElementTag)("p", [
        "card__count-sum"
    ], {});
    countValue.innerText = count;
    cardCount.append(countValue);
    const plus = (0, _elemCreatorJs.createElementTag)("button", [
        "card__count-plus"
    ], {});
    plus.innerText = "+";
    cardCount.append(plus);
    plus.addEventListener("click", ()=>{
        if (size !== undefined) increaseItemCount(id, needSizes, size);
        else increaseItemCount(id, needSizes);
    });
...
function removeCardfromStorage(needSizes, id, size) {
    let cart = JSON.parse(localStorage.getItem("cart"));
    if (needSizes) {
        for(let i = 0; i < cart.length; i++)if (cart[i].id === id && cart[i].size === size) cart.splice(i, 1);
    } else {
        for(let i = 0; i < cart.length; i++)if (cart[i].id === id) cart.splice(i, 1);
    }
    localStorage.setItem("cart", JSON.stringify(cart));
    (0, _basketJs.changeCartAppear)(cart);
}
function increaseItemCount(itemId, needSizes, size) {
    let card;
    if (size) card = document.getElementById(`${itemId}-${size}`);
    else card = document.getElementById(`${itemId}-1`);
    ...
    let cart = JSON.parse(localStorage.getItem("cart"));
    if (needSizes) {
        for (const elem of cart)if (elem.id === itemId && elem.size === size) {
            elem.count++;
            count.innerText = countNum + 1;
            changeCardSum(card, elem.currentPrice, elem.count);
        }
    } else {
        for (const elem of cart)if (elem.id === itemId) {
            elem.count++;
            count.innerText = countNum + 1;
            changeCardSum(card, elem.currentPrice, elem.count);
        }
...
function reduceItemCount(itemId, needSizes, size) {
    ...
    (0, _basketCounterJs.reduceCartCount)();
    localStorage.setItem("cart", JSON.stringify(cart));
    (0, _basketJs.changeTotalSum)();
}
...
},{"./elemCreator.js":"20Jzo","./basket.js":"igkur","./basketCounter.js":"54VTX","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"20Jzo":[function(require,module,exports) {
//Функция создаёт элемент
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "createElementTag", ()=>createElementTag);
function createElementTag(block, cssClass = [], attrs = {}) {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["38iX0","igkur"], "igkur", "parcelRequirecc8e")

//# sourceMappingURL=index.f5bf84e4.js.map
